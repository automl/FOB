...
output_dir="/p/scratch/laionize/franke5/experiments/fob2"
submission="adamw_baseline"

cd $fob_path

# Running the job
start=`date +%s`
#srun python submission_runner.py --workload=$workload --output=$output_dir --data_dir=$data_dir --submission=$submission --hyperparameters "/p/scratch/laionize/franke5/workspace/FOB/icml_related/hyperparameters/adamw_baseline/imagenet64" --workers=16 --seed $seed --trials 1 --start_trial $((first_trial + 3)) --start_hyperparameter 3 --silent --devices=4 --max_steps=50000
