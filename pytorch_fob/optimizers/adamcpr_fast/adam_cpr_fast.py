    ...
    def __init__(
        ...
        *,
        ...
        defaults = dict(
            ...
            betas=betas,
            eps=eps,
            weight_decay=weight_decay if not regularize_all else 1.0,
...
# @torch.jit.script
# def elr_update(param, lagmul, kappa, kappa_update, grad, eps: float = 1e-8):

#     n = param.numel()
#     sum_l2_param = param.square().sum()
#     sum_l2_grad = grad.square().sum()
#     sum_l2_elr = sum_l2_grad / (sum_l2_param + eps)

#     param_specific_lagmul_rate = kappa_update / n
#     param_specific_kappa = kappa * n

#     constraint_value = sum_l2_elr - param_specific_kappa

#     lagmul.add_(param_specific_lagmul_rate * constraint_value).clip_(min=0.)

#     grad_elr = -sum_l2_grad * param
#     # grad_elr = -2 * sum_l2_grad / param.pow(-3)
...
def _multi_tensor_adam(
    ...
    *,
    ...
    for (
        ...
        device_prev_reg_second_derivatives = [
            torch.view_as_real(x) if torch.is_complex(x) else x for x in prev_reg_second_derivatives
        ]
                ...
                if warm_start < device_state_steps[0]:
                    if reg_function == "l2":
                        ...
                        for param_abs, square_params, device_param in zip(abs_params, square_params, device_params):
                            huber_loss_params.append(
                                torch.where(param_abs < 1, 0.5 * square_param, param_abs - 0.5).sum()
                            )
                    ...
                    if reg_function == "l2":
                        ...
                        square_params = torch._foreach_pow(device_params, 2)
                        new_kappas = [
                            torch.where(param_abs < 1, 0.5 * square_param, param_abs - 0.5).mean()
                            for param_abs, square_params in zip(abs_params, square_params)
                        ]
                        torch._foreach_add_(device_kappas, new_kappas)
