...
def neps_to_fob(config: dict) -> dict:
    pass  # TODO: convert neps config from above searchspace to FOB style
...
def run_pipeline(pipeline_directory, previous_pipeline_directory, **config) -> dict:
    ...
    fob_cfg = neps_to_fob(config)
    engine = Engine()
    engine.parse_experiment(fob_cfg)
